...
export function AuthWizard({
  ...
  return (
    <div className={cn(authWizardVariants({ className }))}>
        ...
        {currentStep === AUTH_STEPS.LOGIN && (
          <form onSubmit={handleLoginSubmit(onLogin)} className="space-y-4">
            <div>
              ...
              />
              {loginErrors.email && (
                <p className={formFieldStyles.error}>{loginErrors.email.message}</p>
              )}
            </div>
            ...
            <div>
              ...
              {loginErrors.password && (
                <p className={formFieldStyles.error}>{loginErrors.password.message}</p>
              )}
            </div>
        ...
        {currentStep === AUTH_STEPS.REGISTER && (
          <form onSubmit={handleRegisterSubmit(onRegister)} className="space-y-4">
            <div>
              ...
              />
              {registerErrors.name && (
                <p className={formFieldStyles.error}>{registerErrors.name.message}</p>
              )}
            </div>
            ...
            <div>
              ...
              {registerErrors.email && (
                <p className={formFieldStyles.error}>{registerErrors.email.message}</p>
              )}
            </div>

            <div>
              <label htmlFor="register-password" className={formFieldStyles.label}>
                Password
              </label>
              <input
              ...
              {registerErrors.password && (
                <p className={formFieldStyles.error}>{registerErrors.password.message}</p>
              )}
            </div>

            <div>
              <label htmlFor="confirm-password" className={formFieldStyles.label}>
                Confirm Password
              </label>
              <input
                id="confirm-password"
